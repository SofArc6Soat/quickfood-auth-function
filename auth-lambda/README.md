...
```bash
auth-lambda$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
auth-lambda$ sam local start-api
auth-lambda$ curl http://localhost:3000/
```
...
```bash
auth-lambda$ sam logs -n NetCodeWebAPIServerless --stack-name auth-lambda --tail
```
...
```bash
auth-lambda$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
